<%-- 
    Document   : Book
    Created on : Mar 14, 2023, 12:45:50 AM
    Author     : Acer
--%>
        ...
        <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 drop-shadow-lg shadow-black" aria-label="Sidebar">
            <div class="h-full w-full px-3 py-4 pt-20 flex flex-col justify-between overflow-y-auto bg-gray-100 dark:bg-gray-800">
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="ChooseType.jsp" class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white bg-gray-300 dark:bg-gray-700">
                            <span class="w-8 text-center">
        ...
        <div class="ml-64 pt-16 h-screen flex items-center justify-center">
            <div class="w-full h-full p-10">
                <div class="w-full h-full">
                    <div class="flex">
                        <div class="">
                            <h1 class="font-bold text-3xl text-gray-900 pb-1 mb-8 border-b-2 border-blue-700 inline-block">Available Rooms</h1>
                            <form action="book" method="post">
                                <table>
                                    <tr>
                                        <th>RoomID</th>
                                        <th>Number of slots (type)</th>
                                        <th>Number of available slots</th>
                                        <th>Select</th>
                                    </tr>
                                    <c:forEach items="${roomData}" var="r">
                                        <tr>
                                            <td>${r.getId()}</td>
                                            <td>${r.getNumOfSlots()}</td>
                                            <td>${r.getNumOfAvailableSlots()}</td>
                                            <td class="text-center accent-blue-800">
                                                <input type="radio" name="room" value="${r.getId()}" id="">
                                            </td>
                                        </tr>
                                    </c:forEach>

                                </table>                            
                                <input value="Next" name="chooseroom" class="cursor-pointer flex items-center justify-center text-center mt-4 px-5 py-2 text-sm font-medium rounded-lg bg-blue-700 text-blue-100 hover:bg-blue-800" type="submit">
                            </form>
                        </div>
                    </div>
                </div>
